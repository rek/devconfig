// Same 24-widget gallery as ../quickshell-gallery, but fed fixed demo values
// instead of the live poll — your keyboard reads off|--|--  right now, which
// left most cells rendering empty. components/ is a copy of
// ../quickshell-gallery/components (quickshell won't load QML modules from
// outside its own config folder) — edit a design in both places, or just
// here and port the winner back once picked.
//
// Launch:  quickshell -p ~/dev/devconfig/.config/quickshell-gallery-demo
// Close:   pkill -f 'quickshell -p.*quickshell-gallery-demo'
...
ShellRoot {
    ...
    // Fixed demo state: BLE-linked, left half healthy, right half getting low
    // — asymmetric on purpose so low-battery styling is visible too.
    readonly property string link: "ble"
    readonly property string lpct: "87"
    readonly property string rpct: "23"
    ...
    PanelWindow {
        ...
        WlrLayershell.namespace: "qs-keyboard-hud-gallery-demo"

        Text {
            anchors.top: parent.top
            anchors.horizontalCenter: parent.horizontalCenter
            anchors.topMargin: 10
            text: "keyboard HUD — design gallery (DEMO DATA: " + root.link + " · L " + root.lpct + "% · R " + root.rpct + "%)  ·  pick your favorites"
            color: "#666"
