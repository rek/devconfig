// One-click launcher for Orca Slicer. Sits immediately left of the Steam
// rescue button, bottom-right of eDP-2, matching its size/style so the two
// read as a paired row of utility buttons.
...
PanelWindow {
    ...
    WlrLayershell.namespace: "qs-orca-slicer-launch"
    ...
    function launch() {
        proc.command = ["bash", "-lc",
            "~/.config/eww/scripts/launch.sh uwsm-app -- orca-slicer"];
        proc.running = true;
        hud.flash = true;
        flashOff.restart();
    }
    ...
    Rectangle {
        ...
        anchors {
            ...
            rightMargin: 82   // 24 (Steam's margin) + 46 (Steam's width) + 12 gap
            bottomMargin: 560
        }
        width: 46
        height: 46
