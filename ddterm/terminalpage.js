...
export const TerminalPage = GObject.registerClass({
    ...
    open_hyperlink() {
        const hyperlink = this.terminal.last_clicked_hyperlink;

        if (hyperlink && hyperlink.startsWith('codegoto:')) {
            this._open_file_in_editor(hyperlink.substring('codegoto:'.length));
            return;
        }
    ...
    _open_file_in_editor(fileLine) {
        const match = fileLine.match(/^(.+?):(\d+(?::\d+)?)$/);

        if (!match)
            return;

        const [, pathPart, lineCol] = match;
        ...
        if (!pathPart.startsWith('/') && !pathPart.startsWith('~')) {
            ...
            if (cwd)
                resolvedPath = cwd.resolve_relative_path(pathPart).get_path();
        }

        try {
            const arg = `${resolvedPath}:${lineCol}`;
            GLib.spawn_command_line_async(`code --goto ${GLib.shell_quote(arg)}`);
        } catch (ex) {
            logError(ex, `Failed to open ${fileLine} in editor`);
        }
    }
